...
const foundDateLibs = depNames.filter(
  (d) => DATE_LIBS.includes(d) && !isExcepted(d, 'date-lib')
);
...
const foundLodash = depNames.filter(
  (d) => LODASH_VARIANTS.includes(d) && !isExcepted(d, 'lodash-variant')
);
...
const foundCssInJs = depNames.filter(
  (d) => CSS_IN_JS.includes(d) && !isExcepted(d, 'css-in-js')
);
