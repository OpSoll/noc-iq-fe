...
export function filterPayments<
  T extends { reconciliation: string; createdAt: string },
>(payments: T[], filters: { reconciliation?: string; from?: string }): T[] {
  return payments.filter((p) => {
    if (filters.reconciliation && p.reconciliation !== filters.reconciliation)
      return false;
...
describe('payments route coverage', () => {
  ...
  it('filters by reconciliation state', () => {
    expect(filterPayments(rows, { reconciliation: 'mismatched' })).toHaveLength(
      1
    );
  });
