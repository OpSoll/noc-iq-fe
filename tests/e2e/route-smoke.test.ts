...
const MODES = Object.keys(MODE_RESTRICTIONS) as Array<
  keyof typeof MODE_RESTRICTIONS
>;
...
describe('Route Smoke Matrix', () => {
  ROUTES.forEach((route) => {
    ...
    ROLES.forEach((role) => {
      MODES.forEach((mode) => {
        ...
        const roleAllows =
          ROLE_CAPABILITY_MATRIX[
            role as keyof typeof ROLE_CAPABILITY_MATRIX
          ].includes(capability);
  ...
  it('restricted mode only allows view:dashboard and view:outages', () => {
    ...
    expect(hasCapability('admin', 'action:resolve-outage', 'restricted')).toBe(
      false
    );
  });
});
