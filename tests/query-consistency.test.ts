...
const sharedResources: SharedResource[] = [
  ...
  },
  {
    route: '/webhooks',
    key: 'webhook_list',
    expectedShape: { id: 'string', url: 'string', event_type: 'string' },
  },
];
...
function checkShape(
  ...
  for (const [key, expectedType] of Object.entries(shape)) {
    ...
    if (
      actual !== expectedType &&
      !(data[key] === null && expectedType === 'string')
    ) {
...
describe('Cross-Route Shared Resource Consistency', () => {
  it.each(sharedResources)(
    '$route returns consistent shape for $key',
    ({ key, expectedShape }) => {
      ...
      if (key === 'payment_list')
        Object.assign(mockData, { transaction_hash: '0xabc', amount: '500' });
      if (key === 'webhook_list')
        Object.assign(mockData, {
          url: 'https://example.com/hook',
          event_type: 'outage.created',
        });
