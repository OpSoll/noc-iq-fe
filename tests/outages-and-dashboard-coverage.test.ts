...
export function validateOutageForm(input: {
  severity?: string;
  siteName?: string;
}) {
...
export function filterKpisByMode<T extends { mode: string }>(
  kpis: T[],
  mode: string
): T[] {
...
describe('outage create/edit form validation', () => {
  it('passes with a valid severity and site name', () => {
    expect(
      validateOutageForm({ severity: 'high', siteName: 'DC-1' }).valid
    ).toBe(true);
  });
...
describe('dashboard KPI filtering', () => {
  ...
  it('filters KPI cards by mode', () => {
    expect(filterKpisByMode(kpis, 'compare')).toEqual([
      { id: 'b', mode: 'compare' },
    ]);
  });
