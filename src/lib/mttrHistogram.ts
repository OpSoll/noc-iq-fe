...
export function buildMttrHistogram(
  ...
  const buckets: MttrBucket[] = MTTR_BUCKET_DEFS.map((def) => ({
    ...def,
    count: 0,
  }));
  ...
  for (const outage of outages) {
    ...
    if (!resolvedWithinRange(outage, filters.dateFrom, filters.dateTo))
      continue;
