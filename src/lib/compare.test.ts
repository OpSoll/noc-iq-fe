...
import {
  validateIntervalCompatibility,
  computeDeltas,
  imputeMissingIntervals,
} from './compare';
...
describe('validateIntervalCompatibility', () => {
  it('matches same granularity', () => {
    expect(
      validateIntervalCompatibility(
        { start: 'a', end: 'b', granularity: 'day' },
        { start: 'c', end: 'd', granularity: 'day' }
      ).compatible
    ).toBe(true);
  });
  it('rejects different granularity', () => {
    expect(
      validateIntervalCompatibility(
        { start: 'a', end: 'b', granularity: 'hour' },
        { start: 'c', end: 'd', granularity: 'day' }
      ).compatible
    ).toBe(false);
  });
...
describe('imputeMissingIntervals', () => {
  it('adds missing intervals', () => {
    const result = imputeMissingIntervals(
      [{ period: '2024-01' }],
      ['2024-01', '2024-02']
    );
