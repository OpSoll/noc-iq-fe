...
export function testPayloadAgainstSchema(
  ...
  for (const field of schema) {
    ...
    const passed =
      !field.required || (value !== undefined && actualType === field.type);
    results.push({
      field: field.name,
      passed,
      ...
      error:
        field.required && value === undefined
          ...
          : passed
            ? undefined
            : `Expected ${field.type}, got ${actualType}`,
    });
  }
...
export function formatSchemaTestResult(
  summary: SchemaCompatibilitySummary
): string {
  return `Schema test: ${summary.passed}/${summary.totalFields} passed (${summary.failed} failed)`;
}

export function getFailedFields(
  summary: SchemaCompatibilitySummary
): SchemaTestResult[] {
