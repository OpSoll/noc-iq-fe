...
export function detectContractDrift(
  ...
  for (const [field, expectedType] of Object.entries(expected)) {
    ...
    if (
      ...
      !(actual[field] === null && expectedType === 'string')
    ) {
      warnings.push({
        endpoint: 'unknown',
        field,
...
export function buildDriftReport(
  ...
  const warnings = detectContractDrift(expected, actual).map((w) => ({
    ...w,
    endpoint,
  }));
...
export function formatDriftForAdminPanel(report: DriftReport): string[] {
  return report.warnings.map(
    (w) =>
      `[${w.severity.toUpperCase()}] ${w.endpoint}.${w.field}: expected ${w.expectedType}, got ${w.actualType}`
  );
}
