export function getWalletErrorMessage(error: unknown): string {
  ...
  if (status === 404)
    return 'Wallet not found. Please check your user ID and try again.';
  ...
  if (status === 401 || status === 403)
    return "You don't have permission to manage wallets.";
  if (status === 0 || !status)
...
export function getWalletHealthLabel(status: {
  ...
  if (!status.trustline_ready)
    return { label: 'Trustline Missing', variant: 'warning' };
