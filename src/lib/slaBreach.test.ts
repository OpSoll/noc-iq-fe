...
describe('computeMinutesRemaining', () => {
  it('returns the full threshold when just detected', () => {
    expect(computeMinutesRemaining(NOW.toISOString(), 30, NOW)).toBeCloseTo(
      30,
      5
    );
  });
...
describe('buildBreachCountdowns', () => {
  ...
  it('flags outages under the warning threshold', () => {
    ...
    const outages: Outage[] = [
      makeOutage({ id: '1', detected_at: detectedAt, severity: 'critical' }),
    ];
  ...
  it('sorts soonest-to-breach first', () => {
    const outages: Outage[] = [
      makeOutage({
        id: 'far',
        detected_at: new Date(NOW.getTime() - 5 * 60_000).toISOString(),
        severity: 'critical',
      }), // 25m left
      makeOutage({
        id: 'near',
        detected_at: new Date(NOW.getTime() - 25 * 60_000).toISOString(),
        severity: 'critical',
      }), // 5m left
      makeOutage({
        id: 'breached',
        detected_at: new Date(NOW.getTime() - 35 * 60_000).toISOString(),
        severity: 'critical',
      }), // -5m
    ];
    const countdowns = buildBreachCountdowns(outages, thresholds, NOW);
    expect(countdowns.map((c) => c.outageId)).toEqual([
      'breached',
      'near',
      'far',
    ]);
