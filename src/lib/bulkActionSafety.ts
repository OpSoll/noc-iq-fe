...
export function previewBulkAction(
  action: BulkAction,
  currentOutages: Record<string, { slaStatus: string; hasPayment: boolean }>
): SafetyResult {
  ...
  if (paymentChanges > 5)
    warnings.push(
      `Bulk action affects ${paymentChanges} payments. Consider batching.`
    );
...
export function executeBulkAction(
  action: BulkAction,
  preview: SafetyResult
): { success: boolean; message: string } {
  if (!preview.safe)
    return {
      success: false,
      message: `Cannot execute: ${preview.warnings.join('; ')}`,
    };
  return {
    success: true,
    message: `Applied ${action.type} to ${preview.summary.affectedOutages} outage(s)`,
  };
}
