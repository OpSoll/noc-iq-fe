...
export function checkShapeDrift(
  ...
  for (const [key, expectedType] of Object.entries(spec)) {
    ...
    if (typeof sample[key] !== expectedType) {
      drift.push(
        `type mismatch for ${key}: expected ${expectedType}, got ${typeof sample[key]}`
      );
    }
  }
  return drift;
