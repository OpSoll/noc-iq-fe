  ...
  if (errors.length === 0) {
    ...
    if (parsed.length > 0) {
      ...
      return {
        ...
        rows: [],
        errors: [],
        warnings: [{ message: 'JSON array is empty.' }],
      };
    }
  }
  return { headers, rows, errors, warnings: [] };
        ...
        <div className="space-y-3">
          {preview.errors.length > 0 ? (
            <div className="rounded-lg border border-red-200 bg-red-50 p-3">
              <p className="mb-1 text-sm font-semibold text-red-700">
                {preview.errors.length} blocking error
                {preview.errors.length > 1 ? 's' : ''} — fix before uploading
              </p>
          ...
          {preview.warnings.length > 0 ? (
            <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3">
              <p className="mb-1 text-sm font-semibold text-yellow-700">
                Warnings
              </p>
              <ul className="space-y-0.5">
          ...
          {preview.headers.length > 0 && preview.rows.length > 0 ? (
            <div className="overflow-x-auto rounded-lg border bg-white shadow-sm">
              <p className="border-b px-4 py-2 text-xs font-semibold uppercase tracking-wide text-gray-500">
                Preview (first {preview.rows.length} row
