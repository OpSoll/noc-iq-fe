...
export default function Wave5DashboardPage() {
  ...
  const passing = DOMAINS.filter((d) =>
    [d.testHealth, d.reliabilityScore, d.accessibility].every(
      (s) => s === 'pass'
    )
  ).length;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            FE issue and quality signal tracking —{' '}
        ...
        {[
          ...
          { label: 'Issues Tracked', value: DOMAINS.length },
          {
            ...
            value: DOMAINS.filter((d) =>
              [d.testHealth, d.reliabilityScore, d.accessibility].includes(
                'warn'
              )
            ).length,
          },
            ...
            {DOMAINS.map((row) => (
              <tr key={row.domain} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3">
                  <Badge signal={row.testHealth} />
                </td>
                <td className="px-4 py-3">
                  <Badge signal={row.reliabilityScore} />
                </td>
                <td className="px-4 py-3">
                  <Badge signal={row.accessibility} />
                </td>
