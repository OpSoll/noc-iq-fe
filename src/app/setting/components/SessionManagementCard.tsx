  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm space-y-3">
          ...
          <p className="text-slate-500">
            Ends your current session and clears stored tokens from this
            browser. You will be redirected to the sign-in page.
          </p>
          <button
            onClick={() => void handleSignOut()}
            disabled={
              sessionState !== 'authenticated' || sessionActionLoading !== null
            }
        ...
        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm space-y-3">
          ...
          <p className="text-slate-500">
            Invalidates all active refresh tokens for your account across every
            device. Use this if you suspect unauthorised access.
          </p>
          <button
          ...
          >
            {sessionActionLoading === 'logout-all'
              ? 'Revoking…'
              : 'Revoke all sessions'}
          </button>
        </div>
      ...
      <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800 space-y-1">
        ...
        <p>
          Your access token is short-lived. When it expires the app
          automatically exchanges your refresh token for a new pair — you stay
          signed in without any action required.
        </p>
        <p>
          If the refresh fails (token revoked, network error, or server restart)
          you will see a &ldquo;Session expired&rdquo; message and be redirected
          to sign in again. No data is lost; simply sign back in to continue.
        </p>
      </div>
    </section>
  );
