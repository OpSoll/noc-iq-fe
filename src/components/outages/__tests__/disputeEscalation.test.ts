...
describe("isEscalatable (opsoll/noc-iq-fe#496)", () => {
  it("allows escalation for a pending dispute older than 7 days", () => {
    expect(
      isEscalatable(
        { created_at: createdAtDaysAgo(ESCALATION_THRESHOLD_DAYS + 1), status: "open" },
        now,
      ),
    ).toBe(true);
  });
  ...
  it("is unavailable exactly at the 7-day boundary and before it", () => {
    expect(
      isEscalatable(
        { created_at: createdAtDaysAgo(ESCALATION_THRESHOLD_DAYS), status: "open" },
        now,
      ),
    ).toBe(true);
    expect(
  ...
  it("is unavailable for already-settled disputes", () => {
    for (const status of ["resolved", "rejected"] as const) {
      expect(
        isEscalatable(
          { created_at: createdAtDaysAgo(30), status },
          now,
        ),
      ).toBe(false);
    }
