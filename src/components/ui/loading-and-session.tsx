...
export function SessionTimeoutDialog({
  ...
  useEffect(() => {
    const timer = setTimeout(
      () => setVisible(true),
      Math.max(expiresInMs - warnBeforeMs, 0)
    );
  ...
  return (
    <div
      role="alertdialog"
      aria-live="assertive"
      className="fixed bottom-4 right-4 rounded-lg border bg-background p-4 shadow-lg"
    >
      <p className="text-sm">Your session is about to expire.</p>
      <div className="mt-2 flex gap-2">
        <button onClick={onExtend} className="text-sm font-medium">
          Extend Session
        </button>
