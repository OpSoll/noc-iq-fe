...
export function useReplayGuard(cooldownMs: number = 10_000) {
  ...
  const checkReplay = useCallback(
    (webhookId: string): ReplayGuardState => {
  ...
  const attemptReplay = useCallback(
    (action: ReplayAction): { ok: boolean; reason?: string } => {
      const state = checkReplay(action.webhookId);
      if (!state.canReplay) {
        return {
          ok: false,
          reason: `Cooldown active. Wait ${state.remainingCooldownMs}ms before replaying.`,
        };
      }
